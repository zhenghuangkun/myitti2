 ...
 var columnFour =[
		{
			...
			"render": function (data, type, row, meta) {
				if((row.userId!=row.currentUserId && row.roleType==pfRoleType && row.createBy!=row.currentCreateBy)|| row.userState==0){//平台登录时不是自己创建的不能编辑和删除
					...
					return "<div class='hidden-sm hidden-xs btn-group'>" +
					"<button class='btn btn-xs btn-info' onclick='editUser(" + '"' + row.userId + '"' +")'><i class='ace-icon fa fa-pencil bigger-120'>编辑</i></button>" +
					"<button class='btn btn-xs btn-danger' onclick='deleteUser(" + '"' + row.userId + '"' +")'><i class='ace-icon fa fa-trash-o bigger-120'>禁用</i></button>" +
