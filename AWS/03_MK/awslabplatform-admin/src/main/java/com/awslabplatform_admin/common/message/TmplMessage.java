...
public enum TmplMessage implements Message {
	...
	TMPLATE_RESOURCE_CREATE_FAILURE(10123, "启动失败，未取到可用账号，该用户可能未配账号池"),

	TMPLATE_RESOURCE_CREATE_NULL(10139, "启动失败，获取的堆栈为空，可能未取到可用账号，该用户可能未配账号池"),

	TMPLATE_START_SUCCESS(10124, "资源创建成功"),
	
	TMPLATE_START_ERROR(10125, "资源创建过程中出现错误"),
	
	TMPLATE_RESOURCE_DELETE_ERROR (10126, "堆栈信息不存在或未取到可用账号，该用户可能未配账号池"),
