...
<body>
   <!-- 头部 -->
	<div style="background-color: #23262E;min-height: 60px;">
		<img alt="logo" src="${pageContext.request.contextPath}/resource/images/awslogo.png"
				 style="margin-left: -1068px;margin-top: 12px;">
	</div>
   <!-- 确认账号 -->
      ...
      <div id="data_form">
		<div class="reg_form">
			<form>
				...
				<img id="sendVerCode" onclick="changeImgVerCode(this)" src="${pageContext.request.contextPath}/createLoginCode" alt='验证码' title='点击更换验证码' class='check-img w_5' style='cursor:pointer;'/>
				<!-- <a></a> --><span class="error"></span><span class="correct"></span></dd></dl>
				<div class="btn mt30"><input name="" type="button" class="button" value="下一步"  onclick="checkIdCard()"/></div>
			</form>
          </div>
      </div>
	</div>
   ...
   <!-- 重置密码 -->
	<div class="adm_body w692 none" id="reset" >
		...
		<form>
			...
			<dl>
				<dt>新密码：</dt>
				<dd><input id="newPwd" name="newPwd" type="password" placeholder="密码为6-30位字符组合" onblur="passChangeSuccess()">
   ...
   <!-- 完成 -->
	...
	function checkIdCard(){
		...
		return false;      	
			...
			if(account){
				 $.ajax({
					 // TODO
		             type: "POST",
		             url: "${pageContext.request.contextPath}/confirmAccount",
		...
		function changeImgVerCode() {
			$("#sendVerCode").attr("src","${pageContext.request.contextPath}/createLoginCode?timestamp=" + (new Date()).valueOf());
		}
		...
		/* 发送验证码 */
		function sendCode(){
			/* var mod= $('.select-toggle').find('li.select-list.on').attr("mod"); */
			var mod = $("#account").val();
			var btn = $("#codeBtn");
			...
			$.ajax({
				type: "POST",
	            url: "${pageContext.request.contextPath}/sendForgetPwdEmailVerCode",
	            data: {Email:mod},
	            dataType: "json",
	            success: function(data){
	            	if(data.resultFlag){
        ...
        function checkCode(){
			...
			if(!verCode){
				...
				$.ajax({
		             type: "POST",
		             url: "${pageContext.request.contextPath}/securityVerification",
